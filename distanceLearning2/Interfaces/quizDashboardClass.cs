...
//Contensive.Addons.DistanceLearning3.quizDashboardClass
{
                ...
                else
                {
                    ...
                    while (cs.OK())
                    {
                       ...
                       // if (userName.ToLower() == "guest") { userName += " #" + cs.GetInteger("userId"); }
                        ...
                        reportList.setCell(statics.getShortDateString(cs.GetDate("DateCreated")));
                        cs.GoNext();
                    }
                }
                cs.Close();
                inputForm = "<div class=\"afwTextAlignRight\"><input type=\"text\" name=\"QuizName\" placeholder=\"Name-for-Quiz\" value=\"\">"
                    + cp.Html.Button("button", statics.rnbuttonInputNewQuiz, "addQuizClass","js-addQuizButtonId")
                    + "</div></br>";
                reportList.columnCellClass = "afwTextAlignRight";
