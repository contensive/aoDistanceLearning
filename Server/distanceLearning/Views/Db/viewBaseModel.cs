        ...
        public static T create<T>(CPBaseClass cp, QuizModel settings) where T : ViewBaseModel
        {
            ...
            try
            {
                ...
                result.outerContainerClass = ""
                    + (settings.fontStyleId == 0 ? string.Empty : " " + Contensive.Models.Db.DbBaseModel.getRecordName<Db.DesignBlockFontModel>(cp, settings.fontStyleId))
                    + (settings.themeStyleId.Equals(0) ? string.Empty : " " + Models.Db.DesignBlockThemeModel.getRecordName<Models.Db.DesignBlockThemeModel>(cp, settings.themeStyleId))
                    + "";
