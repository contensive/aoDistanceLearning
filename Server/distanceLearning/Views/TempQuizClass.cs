...
namespace Contensive.Addons.DistanceLearning {
    // 
    public class TempQuizClass : AddonBaseClass {
        // 
        ...
        public List<QuizQuestionModel> questions;
        ...
        public override object Execute(CPBaseClass cp) {
            ...
            try {
                ...
                quiz = QuizModel.create(cp, requestQuizGuid);
                if ((quiz == null)) {
                    //
            ...
            try {
                ...
                foreach (DistanceLearning.Models.QuizResponseDetailModel responseDetail in responseDetailList) {
                    if (responseDetail.pageNumber == response.lastPageNumber) {
                        ...
                        // 
                        // -- Add Question
                        q = q + "" + "" + "<div class=\"questionText\">" + question.copy + "</div>";
                        ...
                        if ((answerList == null))
                            ...
                            foreach (DistanceLearning.Models.QuizAnswerModel answer in answerList) {
                                ...
                                if ((answer.id == responseDetail.answerId))
                                    ...
                                    htmlRadio = htmlRadio + ">";
                                string innerHtml = "" + "" + "<div class=\"questionChoice\">" + htmlRadio + "" + quizEditIcon + "<div class=\"quizanswerClass\">" + answerCopy + "</div></div>";
                                if (cp.User.IsEditingAnything) {
                                    innerHtml = GenericController.addEditWrapper(cp, innerHtml, answer.id, answer.name, "Quiz Answers");
                                }
                                q = q + innerHtml;
                                answerCnt = answerCnt + 1;
                            }
                        if (cp.User.IsEditingAnything) {
                            q += cp.Content.GetAddLink("Quiz Answers", "questionid=" + question.id);
                        }
                        hint = 80;
                        // 
                ...
                if (cp.User.IsEditingAnything) {
                    returnHtml += cp.Content.GetAddLink("Quiz Questions", "quizid=" + quiz.id + ", pageOrder=" + rnDstPageOrder);
                }
                // 
