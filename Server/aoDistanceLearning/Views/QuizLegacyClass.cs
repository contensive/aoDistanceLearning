            ...
            try {
                ...
                if ((quiz == null)) {
                    ...
                    if (!DistanceLearning.Controllers.GenericController.isDateEmpty(response.dateSubmitted)) {
                        ...
                        LegacyQuizViewModel legacyQuizViewModel = LegacyQuizViewModel.create(cp, quiz, response);

                        CPCSBaseClass cs = cp.CSNew();
                        cs.OpenRecord("Layouts", quiz.layoutId);

                        returnHtml = Nustache.Core.Render.StringToString(cs.GetText("layout"), legacyQuizViewModel);
                ...
                foreach (DistanceLearning.Models.QuizQuestionModel question in questionList) {
                    ...
                    if (SelectedAnswerID != 0) {
                        ...
                        if ((responseDetails == null)) {
                            responseDetails = DistanceLearning.Models.QuizResponseDetailModel.addDefault(cp);
